...
for f in "$REPO/supabase/tests/00_commission_stubs.sql" \
         ...
         "$REPO/supabase/migrations/20260730100004_card_commission_production_pipeline.sql" \
         "$REPO/supabase/migrations/20260730100005_card_commission_review_and_balance.sql" \
         "$REPO/supabase/migrations/20260730100006_card_commission_refunds.sql"; do
  [[ -f "$f" ]] || continue
  printf -- '── %s\n' "$(basename "$f")"
