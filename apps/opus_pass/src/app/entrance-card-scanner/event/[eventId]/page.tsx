...
import { validateScannerSession } from '@/lib/scanner/api/checkin'
...
export default function EventGatePage({
  ...
  useEffect(() => {
    ...
    if (!token) {
      if (session && session.eventId === eventId) router.replace(`/entrance-card-scanner/event/${eventId}/scan`)
      else router.replace('/entrance-card-scanner')
      return
    }
    ...
    validateScannerSession(eventId, token)
      .then((validated) => {
        ...
        if (!validated.ok) {
          setState('error')
          setError(validated.error || t('link_not_valid'))
          return
        }
        ...
        if (validated.attendantName) {
          ...
          router.replace(`/entrance-card-scanner/event/${eventId}/scan`)
          return
        }
      ...
      <div className="mt-10 w-full max-w-sm">
        {state === 'checking' ? (
          <div className="flex items-center justify-center gap-3 text-[#1A1A1A]">
            ...
            <p className="text-sm text-[#1A1A1A]/70">{t('verifying_link')}</p>
          </div>
        ) : null}
        ...
        {state === 'error' ? (
          <div className="text-center">
            ...
            <h1 className="mt-5 text-2xl font-bold tracking-tight text-[#1A1A1A]">{t('link_not_valid')}</h1>
            <p className="mt-3 text-sm text-[#1A1A1A]/60">{error}</p>
            <button
              type="button"
              onClick={() => router.replace('/entrance-card-scanner')}
        ...
        {state === 'attendant' ? (
          <form onSubmit={startShift}>
            <p className="text-[11px] font-semibold tracking-wide text-[#8e57b3] uppercase">
              {pending?.eventName ?? t('event')}
            </p>
            <h1 className="mt-2 text-2xl font-bold tracking-tight text-[#1A1A1A]">{t('whos_scanning')}</h1>
            <p className="mt-3 text-sm text-[#1A1A1A]/60">
              {t('start_shift_at', { door: pending?.doorLabel ?? '' })}
            </p>
