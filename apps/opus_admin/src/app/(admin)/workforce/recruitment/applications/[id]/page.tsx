...
import {
  addApplicationNote,
  approveApplicationMessage,
  composeApplicationMessage,
  createAssessment,
  createInterview,
  createOffer,
...
const ACTIVE_STAGES = [
  ...
  ['reference_check', 'References / checks'],
  ['offer', 'Offer'],
  ['on_hold', 'On hold'],
];
...
export default async function ApplicationDetailPage({
  ...
  const access = await requireRecruitmentAccess({
    ...
    entityId: id,
    allowedPermissions: ['workforce.applications.read'],
  });
  const supabase = createSupabaseAdminClient();
  let notesQuery = supabase
    .from('recruitment_candidate_notes')
    .select(
      'id, body, visibility, created_at, author_employee_id, workforce_employees(full_name)'
    )
    .eq('application_id', id)
    .order('created_at', { ascending: false });
  notesQuery = access.employeeId
    ? notesQuery.or(
        `visibility.neq.private,author_employee_id.eq.${access.employeeId}`
      )
  ...
  const [
    ...
    supabase
      ...
      .select(
        '*, recruitment_candidates(*), workforce_jobs(id, title, department, location, slug)'
      )
      .eq('id', id)
      .maybeSingle(),
    supabase
      .from('recruitment_application_stage_history')
      .select(
        'id, from_status, to_status, candidate_facing_status, reason_code, note, changed_by_actor_type, created_at'
      )
      .eq('application_id', id)
      .order('created_at', { ascending: false }),
    notesQuery,
    supabase
      ...
      .select('id, question_label, answer')
      .eq('application_id', id)
      .order('created_at'),
    supabase
      .from('recruitment_interviews')
      .select('id, title, status, starts_at, interview_type')
      .eq('application_id', id)
      .order('starts_at'),
    supabase
      .from('recruitment_assessments')
      .select('id, title, status, score, max_score, due_at')
      .eq('application_id', id)
      .order('created_at'),
    supabase
      .from('recruitment_offers')
      .select(
        'id, offer_number, status, version, start_date, expires_at, base_salary, currency'
      )
      .eq('application_id', id)
      .order('created_at', { ascending: false }),
    supabase
    ...
    supabase
      ...
      .select(
        'id, channel, direction, subject, body, status, scheduled_for, approval_status, sent_by, created_at, recruitment_message_recipients(address, delivery_status)'
      )
      .eq('application_id', id)
      .order('created_at', { ascending: false })
      .limit(100),
    ...
    supabase
      ...
      .select(
        'id, referee_name, referee_email, relationship, status, response, requested_at, received_at, access_expires_at'
      )
      .eq('application_id', id)
      .order('created_at', { ascending: false }),
    supabase
      ...
      .select(
        'id, check_type, provider, status, candidate_consent_at, result_summary, completed_at'
      )
      .eq('application_id', id)
      .order('created_at', { ascending: false }),
  ...
  const canSeeSensitive =
    permissions.has('workforce.candidates.export') ||
    permissions.has('recruitment.candidate.sensitive');
  const canSeeCompensation = permissions.has(
    'workforce.offers.compensation_read'
  );
  const canScheduleInterview = permissions.has('workforce.interviews.schedule');
  const canCreateOffer =
    permissions.has('workforce.offers.create') &&
    (offerResult.data?.length ?? 0) === 0 &&
    ['final_interview', 'reference_check', 'offer'].includes(
      application.status
    );
  ...
  const canManageChecks = permissions.has('workforce.offers.approve');
  ...
  return (
    <>
      <WorkforceHeading
        ...
        subtitle={`${application.application_reference} · ${job?.title ?? 'Job'} · ${application.candidate_facing_status}`}
      />
      <div className="grid gap-5 xl:grid-cols-[1fr_380px]">
        <div className="space-y-5">
          <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Candidate sees: {application.candidate_facing_status}
                </p>
              </div>
            ...
            <dl className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                ['Current role', candidate?.current_position ?? 'Not provided'],
                [
                  'Organization',
                  candidate?.current_organization ?? 'Not provided',
                ],
                [
                  'Experience',
                ...
                [
                  'Work authorized',
                  application.work_authorized == null
                    ? 'Not answered'
                ...
                [
                  ...
                  application.weekend_available == null
                    ? 'Not answered'
                    : application.weekend_available
                      ? 'Yes'
                      : 'No',
                ],
              ].map(([label, value]) => (
            ...
            {canSeeSensitive && (
              <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4">
                ...
                <p className="mt-2 text-sm text-amber-950">
                  Salary expectation:{' '}
                  {application.salary_expectation ?? 'Not provided'} · Earliest
                  start: {application.earliest_start_date ?? 'Not provided'}
                </p>
              </div>
            )}
          </section>
          ...
          <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            ...
            {canScheduleInterview && (
              <form
                action={createInterview.bind(null, id)}
                className="mt-4 grid gap-2 rounded-xl bg-[#F8EDFF] p-3 sm:grid-cols-[1fr_160px_auto]"
              >
                <input
                  name="title"
                  required
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {(interviewResult.data ?? []).map((interview) => (
                <Link
                  key={interview.id}
                  href={`/workforce/recruitment/interviews/${interview.id}`}
                  className="rounded-xl bg-gray-50 p-4 hover:bg-[#F8EDFF]"
                >
                  <p className="font-semibold text-gray-900">
                    {interview.title}
                  </p>
                  <p className="mt-1 text-xs capitalize text-gray-500">
                    {interview.status} · {interview.interview_type}
                  </p>
              ...
              {(assessmentResult.data ?? []).map((assessment) => (
                <Link
                  key={assessment.id}
                  href={`/workforce/recruitment/assessments/${assessment.id}`}
                  className="rounded-xl bg-gray-50 p-4 hover:bg-sky-50"
                >
                  <p className="font-semibold text-gray-900">
                    {assessment.title}
                  </p>
                  ...
                  <p className="mt-2 text-sm text-gray-600">
                    {assessment.score == null
                      ? assessment.due_at
                        ? `Due ${new Date(assessment.due_at).toLocaleDateString('en-TZ')}`
                        : 'Awaiting assignment'
            ...
            {interviewResult.data?.length === 0 &&
              assessmentResult.data?.length === 0 && (
                <p className="mt-4 text-sm text-gray-400">
                  No interviews or assessments yet.
                </p>
              )}
          </section>
          {canCreateAssessment && (
            <section className="rounded-2xl border border-sky-100 bg-sky-50 p-5">
              <h2 className="font-semibold text-sky-950">Request assessment</h2>
              <form
              ...
              >
                ...
                >
                  ...
                  <option>Portfolio submission</option>
                  <option>Presentation</option>
                  <option>Case study</option>
                  <option>Event planning simulation</option>
                  <option>Video response</option>
                  <option>Data exercise</option>
                  <option>Practical field assessment</option>
                </select>
                <input
                  name="due_on"
                  type="date"
          ...
          <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Reference links are one-time and expire after 14 days. Background
              checks cannot start without the candidate&apos;s recorded portal
              consent.
            </p>
            {canAdvance && (
              <form
                action={requestReferenceCheck.bind(null, id)}
                className="mt-4 grid gap-2 rounded-xl bg-[#F8EDFF] p-4 sm:grid-cols-2"
              >
                <input
                  name="referee_name"
                  required
                ...
                <input
                  name="relationship"
                  required
                  placeholder="Working relationship"
                  className="rounded-lg border px-3 py-2 text-sm"
                />
                <select
                  name="language_code"
                  className="rounded-lg border px-3 py-2 text-sm"
                >
                  <option value="en">English request</option>
                  <option value="sw">Kiswahili request</option>
                </select>
                <label className="flex items-start gap-2 text-xs leading-5 sm:col-span-2">
                  <input
                    name="candidate_consent_confirmed"
                    type="checkbox"
                    required
                    className="mt-1"
                  />
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {(referencesResult.data ?? []).map((reference) => (
                <article key={reference.id} className="rounded-xl border p-4">
                  <div className="flex justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {reference.relationship} · {reference.referee_email}
                      </p>
                    </div>
                  ...
                  {['received', 'reviewed'].includes(reference.status) && (
                    <pre className="mt-3 overflow-x-auto whitespace-pre-wrap rounded-lg bg-gray-50 p-3 text-xs text-gray-600">
                      {JSON.stringify(reference.response, null, 2)}
                    </pre>
                  )}
                  {canAdvance && reference.status === 'received' && (
                    <form
            ...
            {canManageChecks && (
              <form
                action={requestBackgroundCheck.bind(null, id)}
                className="mt-5 grid gap-2 rounded-xl bg-amber-50 p-4 sm:grid-cols-3"
              >
                <input
                  name="check_type"
                  required
                  placeholder="Check type, e.g. right to work"
            ...
            <div className="mt-4 space-y-3">
              {(backgroundChecksResult.data ?? []).map((check) => (
                <article key={check.id} className="rounded-xl border p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {check.provider || 'Internal review'} · consent{' '}
                        {check.candidate_consent_at
                          ? 'recorded'
                          : 'not recorded'}
                      </p>
                    </div>
                  ...
                  {canManageChecks &&
                    !['clear', 'failed', 'cancelled'].includes(
                      check.status
                    ) && (
                      <form
                        action={updateBackgroundCheck.bind(null, id, check.id)}
          ...
          <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            <h2 className="text-xs font-bold uppercase tracking-wider text-gray-500">Notes</h2>
            {canNote && (
              <form action={noteAction} className="mt-4 space-y-2">
                ...
                <div className="flex justify-between gap-3">
                  ...
                  >
                    <option value="recruiting_team">Recruiting team</option>
            ...
            <div className="mt-4 divide-y divide-gray-100">
              {(notesResult.data ?? []).map((note) => {
                const author = Array.isArray(note.workforce_employees)
                  ? note.workforce_employees[0]
                  : note.workforce_employees;
                return (
                  <article key={note.id} className="py-4">
                    <div className="flex justify-between gap-3 text-xs text-gray-400">
                      ...
                      <time>
                        {new Date(note.created_at).toLocaleString('en-TZ')}
                      </time>
                    </div>
          ...
          <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Every outbound message and delivery state remains attached to this
              application.
            </p>
            {canCommunicate && (
              <form
                action={composeApplicationMessage.bind(null, id)}
                className="mt-4 grid gap-2 rounded-xl bg-gray-50 p-4 sm:grid-cols-2"
              >
                <select
                  name="channel"
                ...
                >
                  ...
                  <option value="sms">SMS (enabled + consent)</option>
                  <option value="whatsapp">
                    WhatsApp (integrated + consent)
                  </option>
                ...
                <textarea
                  ...
                  required
                  rows={5}
                  placeholder="Candidate-facing message or call summary"
                ...
                >
                  <div className="flex flex-wrap justify-between gap-2">
                    ...
                    <span className="text-xs capitalize text-gray-500">
                      {message.status} · approval {message.approval_status}
                    </span>
                  </div>
                  ...
                  <p className="mt-2 text-xs text-gray-400">
                    {new Date(message.created_at).toLocaleString('en-TZ')}
                    {message.scheduled_for
                      ? ` · scheduled ${new Date(message.scheduled_for).toLocaleString('en-TZ')}`
                      : ''}
                  </p>
                  {canCommunicate &&
                    message.approval_status === 'pending' &&
                    message.sent_by !== access.employeeId && (
                      <div className="mt-3 flex gap-2">
                        <form
                          action={approveApplicationMessage.bind(
                            null,
                            id,
                            message.id,
                            'approved'
        ...
        <aside className="space-y-5">
          {canAdvance && (
            <section className="rounded-2xl border border-[#E8D4F1] bg-[#FBF7FD] p-5 shadow-sm">
              <h2 className="text-xs font-bold uppercase tracking-wider text-gray-500">Move application</h2>
              <form action={transitionAction} className="mt-4 space-y-3">
                <select
                  name="target_status"
                  className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm"
                >
                  {ACTIVE_STAGES.map(([value, label]) => (
          ...
          {canReject && (
            <section className="rounded-2xl border border-rose-200 bg-rose-50 p-5 shadow-sm">
              ...
              <p className="mt-1 text-xs text-rose-800">
                The outcome message will wait for approval by another team
                member.
              </p>
              <form action={transitionAction} className="mt-4 space-y-3">
                ...
                >
                  <option value="">Choose internal reason</option>
                  {(reasonsResult.data ?? []).map((reason) => (
                ...
                >
                  ...
                  {(outcomeTemplatesResult.data ?? []).map((template) => (
          ...
          <section className="rounded-2xl border border-gray-100 bg-white p-5 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            <h2 className="text-xs font-bold uppercase tracking-wider text-gray-500">Offers</h2>
            <div className="mt-3 space-y-3">
              {(offerResult.data ?? []).map((offer) => (
                <Link
                  key={offer.id}
                  href={`/workforce/recruitment/offers/${offer.id}`}
                  className="block rounded-xl bg-gray-50 p-3 hover:bg-[#F8EDFF]"
                >
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    Version {offer.version} · starts {offer.start_date ?? 'TBD'}
                  </p>
                  {canSeeCompensation && (
          ...
          {canCreateOffer && (
            <section className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5 shadow-sm">
              ...
              <form
                action={createOffer.bind(null, id)}
              ...
              >
                ...
                <label className="block text-xs font-semibold text-emerald-900">
                  ...
                  <input
                    name="working_hours"
                    defaultValue="40 hours per week"
                    className="mt-1 w-full rounded-lg border border-emerald-200 bg-white px-3 py-2 text-sm"
                  />
                </label>
                <label className="block text-xs font-semibold text-emerald-900">
                  ...
                  <input
                    ...
                    placeholder="For example: six months"
                    className="mt-1 w-full rounded-lg border border-emerald-200 bg-white px-3 py-2 text-sm"
                  />
                </label>
                <label className="block text-xs font-semibold text-emerald-900">
