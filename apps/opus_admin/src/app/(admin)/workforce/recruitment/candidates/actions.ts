...
export async function mergeCandidates(formData: FormData): Promise<void> {
  const survivingId = text(formData, 'surviving_candidate_id');
  const mergedId = text(formData, 'merged_candidate_id');
  const reason = text(formData, 'reason');
  const access = await requireRecruitmentAccess({
    entityType: 'candidate',
    entityId: survivingId,
    allowedPermissions: ['workforce.candidates.merge'],
  });
  ...
  const { error } = await createSupabaseAdminClient().rpc(
    'recruitment_merge_candidates',
    {
      p_surviving_candidate_id: survivingId,
      p_merged_candidate_id: mergedId,
      p_actor_employee_id: access.employeeId,
      p_reason: reason,
    }
  );
  if (error) throw error;
  revalidatePath('/workforce/recruitment/candidates');
  revalidatePath(`/workforce/recruitment/candidates/${survivingId}`);
  redirect(`/workforce/recruitment/candidates/${survivingId}`);
}

export async function reviewDuplicateMatch(
  id: string,
  status: 'confirmed_duplicate' | 'not_duplicate'
): Promise<void> {
  const { data: match, error: lookupError } = await createSupabaseAdminClient()
    .from('recruitment_duplicate_matches')
    .select('candidate_id, possible_duplicate_id')
    .eq('id', id)
    .single();
  if (lookupError) throw lookupError;
  const access = await requireRecruitmentAccess({
    entityType: 'candidate',
    entityId: match.candidate_id,
    allowedPermissions: ['workforce.candidates.merge'],
  });
  await requireRecruitmentAccess({
    entityType: 'candidate',
    entityId: match.possible_duplicate_id,
    allowedPermissions: ['workforce.candidates.merge'],
  });
...
export async function openCandidateDocument(
  ...
  try {
    ...
    const { data: document, error } = await supabase
      ...
      .select(
        'id, candidate_id, document_class, storage_bucket, storage_path, original_filename, malware_scan_status'
      )
      .eq('id', documentId)
    ...
    if (error) throw error;
    if (!document || document.malware_scan_status !== 'clean') {
    ...
    const restricted = [
      ...
      'employee_transfer',
      'restricted',
    ].includes(document.document_class);
    if (restricted) {
    ...
    const { error: accessError } = await supabase
      .from('recruitment_document_access_events')
      .insert({
    ...
    logDbError('recruitment.candidate_document.open', error, {
      candidateId,
      documentId,
    });
    return { ok: false, error: NOT_VISIBLE };
...
export async function addCandidateNote(
  ...
  try {
    ...
    if (body.length < 1 || body.length > 5000)
      return { ok: false, message: 'Enter a note of up to 5,000 characters.' };
