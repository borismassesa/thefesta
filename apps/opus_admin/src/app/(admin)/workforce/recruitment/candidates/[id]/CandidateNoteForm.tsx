...
export default function CandidateNoteForm({ candidateId }: { candidateId: string }) {
  ...
  return (
    <form action={action} className="mt-4 space-y-3">
      <textarea name="body" required maxLength={5000} rows={3} placeholder="Add evidence-based recruiting context…" className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:border-[#C9A0DC] focus:ring-2 focus:ring-[#E8D4F1]" />
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button data-opus-button="control" disabled={pending} className="opus-button opus-button--primary opus-button--small">{pending ? 'Saving…' : 'Add note'}</button>
      </div>
