...
function fmt(value: string | null): string {
  return value
    ? new Date(value).toLocaleDateString('en-TZ', {
        day: 'numeric',
        month: 'short',
        year: 'numeric',
      })
...
export default async function CandidateDetailPage({
  ...
  const canSensitive =
    ...
    permissions.has('platform.admin');
  ...
  let documentsQuery = supabase
    ...
    .select(
      'id, document_type, document_class, original_filename, mime_type, byte_size, malware_scan_status, retention_until, created_at'
    )
    .eq('candidate_id', id)
    .order('created_at', { ascending: false });
  if (!canSensitive)
  ...
  const [
    ...
    supabase
      ...
      .select(
        'id, job_id, application_reference, status, candidate_facing_status, source, submitted_at, created_at'
      )
      .eq('candidate_id', id)
      .order('created_at', { ascending: false }),
    supabase
      ...
      .select(
        'id, organization, title, location, started_on, ended_on, is_current, description'
      )
      .eq('candidate_id', id)
      .order('sort_order'),
    supabase
      ...
      .select(
        'id, institution, qualification, field_of_study, started_on, ended_on'
      )
      .eq('candidate_id', id)
      .order('sort_order'),
    supabase
      .from('recruitment_candidate_skills')
      .select('skill, proficiency, years_experience')
      .eq('candidate_id', id)
      .order('skill'),
    documentsQuery,
    supabase
      ...
      .select(
        'id, consent_type, notice_version, granted_at, withdrawn_at, source'
      )
      .eq('candidate_id', id)
      .order('created_at', { ascending: false }),
    supabase
      .from('recruitment_candidate_tags')
      .select('tag_id, recruitment_tags(name, color)')
      .eq('candidate_id', id),
    notesQuery,
  ]);
  const results = [
  ...
  return (
    <>
      <WorkforceHeading
        ...
        subtitle={`${candidate.current_position ?? 'Candidate'}${candidate.current_organization ? ` · ${candidate.current_organization}` : ''}`}
      />
      <div className="grid gap-5 xl:grid-cols-[1fr_360px]">
        <div className="space-y-5">
          <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                <span className="rounded-2xl bg-[#F7EAFB] p-3 text-[#5B2D8E]">
                  <UserRound className="h-6 w-6" />
                </span>
                <div>
                  <h2 className="text-xs font-bold uppercase tracking-wider text-gray-500">Profile</h2>
                  <p className="text-sm text-gray-500">
                    {candidate.preferred_name
                      ? `Prefers ${candidate.preferred_name}`
                      : 'No preferred name recorded'}
                  </p>
                </div>
              </div>
            ...
            <dl className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div>
                ...
                <dd className="mt-1 text-sm text-gray-700">
                  {candidate.source_summary ?? 'Direct'}
                </dd>
              </div>
              <div>
                <dt className="text-xs font-semibold uppercase tracking-wide text-gray-400">
                ...
                <dd className="mt-1 text-sm text-gray-700">
                  {fmt(candidate.last_activity_at)}
                </dd>
              </div>
            </dl>
          ...
          <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xs font-bold uppercase tracking-wider text-gray-500">Applications</h2>
            </div>
              ...
              {applications.map((application) => {
                ...
                return (
                  <Link
                    ...
                    className="flex items-center justify-between gap-4 py-4 hover:text-[#5B2D8E]"
                  >
                    <div>
                      ...
                      <p className="mt-1 text-xs text-gray-500">
                        ...
                        {job?.department ?? ''} · {application.source}
                      </p>
                    </div>
          ...
          <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            <div className="flex items-center gap-2">
              <History className="h-5 w-5 text-[#5B2D8E]" />
              <h2 className="text-xs font-bold uppercase tracking-wider text-gray-500">Experience</h2>
            </div>
            <div className="mt-4 space-y-4">
              {(experienceResult.data ?? []).map((item) => (
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    {fmt(item.started_on)} –{' '}
                    {item.is_current ? 'Present' : fmt(item.ended_on)}
              ...
              {experienceResult.data?.length === 0 && (
          ...
          <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            <div className="flex items-center gap-2">
              <GraduationCap className="h-5 w-5 text-[#5B2D8E]" />
            ...
            <div className="mt-4 space-y-3">
              {(educationResult.data ?? []).map((item) => (
                <article key={item.id}>
                  <h3 className="text-sm font-semibold text-gray-900">
                    {item.qualification ?? 'Study'}
                    {item.field_of_study ? `, ${item.field_of_study}` : ''}
                  </h3>
                  <p className="text-xs text-gray-500">
                    {item.institution} · {fmt(item.started_on)} –{' '}
                    {fmt(item.ended_on)}
                  </p>
                </article>
              ))}
        ...
        <aside className="space-y-5">
          <section className="rounded-2xl border border-gray-100 bg-white p-5 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            <div className="flex items-center gap-2">
              <FileLock2 className="h-5 w-5 text-[#5B2D8E]" />
              <h2 className="text-xs font-bold uppercase tracking-wider text-gray-500">Documents</h2>
            </div>
            <p className="mt-1 text-xs leading-5 text-gray-500">
              Links expire after 60 seconds. Every open is recorded.
            </p>
            <div className="mt-4 divide-y divide-gray-100">
              {(documentsResult.data ?? []).map((document) => (
                ...
                >
                  ...
                  <CandidateDocumentButton
                    candidateId={id}
                    documentId={document.id}
                    disabled={document.malware_scan_status !== 'clean'}
                  />
                </div>
              ))}
          ...
          <section className="rounded-2xl border border-gray-100 bg-white p-5 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            <div className="flex items-center gap-2">
              <Tags className="h-5 w-5 text-[#5B2D8E]" />
              <h2 className="text-xs font-bold uppercase tracking-wider text-gray-500">Tags and consent</h2>
            </div>
            <div className="mt-4 flex flex-wrap gap-2">
              {(tagsResult.data ?? []).map((row) => {
                const tag = Array.isArray(row.recruitment_tags)
                  ? row.recruitment_tags[0]
                  : row.recruitment_tags;
                return tag ? (
                  <span
                    key={row.tag_id}
              ...
              {(consentsResult.data ?? []).map((consent) => (
                ...
                >
                  <span className="font-semibold capitalize">
                    {consent.consent_type.replaceAll('_', ' ')}
                  </span>{' '}
                  ·{' '}
                  {consent.withdrawn_at
                    ? `withdrawn ${fmt(consent.withdrawn_at)}`
                    : `granted ${fmt(consent.granted_at)}`}
                </li>
              ))}
              {consentsResult.data?.length === 0 && (
                <li className="text-sm text-gray-400">No consent record.</li>
              )}
            </ul>
          </section>

          <section className="rounded-2xl border border-gray-100 bg-white p-5 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            <h2 className="text-xs font-bold uppercase tracking-wider text-gray-500">Recruitment notes</h2>
            <CandidateNoteForm candidateId={id} />
            <div className="mt-4 divide-y divide-gray-100">
              {(notesResult.data ?? []).map((note) => {
                const employee = Array.isArray(note.workforce_employees)
