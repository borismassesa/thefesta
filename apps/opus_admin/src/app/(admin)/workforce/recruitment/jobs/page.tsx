...
export default async function RecruitmentJobsPage() {
  ...
  return (
    <>
      ...
      <section className="mb-6 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{(postings.data ?? []).map((posting) => { const job = Array.isArray(posting.workforce_jobs) ? posting.workforce_jobs[0] : posting.workforce_jobs; return <Link key={posting.id} href={`/workforce/recruitment/jobs/${posting.id}`} className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm hover:border-violet-200"><div className="flex justify-between gap-3"><h2 className="font-semibold">{posting.public_title}</h2><span className="text-xs capitalize text-gray-500">{posting.status}</span></div><p className="mt-2 text-sm text-gray-500">{job?.department} · {job?.location} · {posting.visibility}</p><p className="mt-3 text-xs text-gray-400">{posting.publish_at ? `Publishes ${new Date(posting.publish_at).toLocaleString('en-TZ')}` : 'Manual publication'}{posting.unpublish_at ? ` · closes ${new Date(posting.unpublish_at).toLocaleString('en-TZ')}` : ''}</p></Link> })}</section>
