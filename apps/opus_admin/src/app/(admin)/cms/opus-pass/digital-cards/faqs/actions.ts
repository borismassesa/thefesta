...
import type { OpusPassDigitalCardsFaqsContent } from '@/lib/cms/opus-pass-digital-cards-faqs'

const PAGE_KEY = 'opus-pass-invitations'
const SECTION_KEY = 'faqs'

export async function saveOpusPassDigitalCardsFaqsDraft(
  draft: OpusPassDigitalCardsFaqsContent,
): Promise<void> {
  ...
  revalidatePath('/cms/opus-pass/digital-cards/faqs')
}

export async function publishOpusPassDigitalCardsFaqs(): Promise<void> {
...
export async function discardOpusPassDigitalCardsFaqsDraft(): Promise<void> {
  await requirePermission('cms.write')
  const supabase = createSupabaseAdminClient()
  const { error } = await supabase
    .from('website_page_sections')
