...
export async function saveOpusPassDigitalCardsFeaturedSuiteDraft(
  draft: OpusPassDigitalCardsFeaturedSuiteContent,
): Promise<void> {
  ...
  revalidatePath('/cms/opus-pass/digital-cards/featured-suite')
}

export async function publishOpusPassDigitalCardsFeaturedSuite(): Promise<void> {
...
export async function discardOpusPassDigitalCardsFeaturedSuiteDraft(): Promise<void> {
  await requirePermission('cms.write')
  const supabase = createSupabaseAdminClient()
  const { error } = await supabase
    .from('website_page_sections')
