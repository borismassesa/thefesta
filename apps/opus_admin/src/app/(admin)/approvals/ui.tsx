...
import type { ApprovalCategory, ApprovalRequest, ApprovalStatus } from './types'
...
export function CategoryChip({
  ...
  if (!category) {
    return (
      <span className="inline-flex w-fit items-center rounded-full bg-gray-100 px-2 py-0.5 text-[11px] font-semibold text-gray-500">
      ...
      {options.map((o) => (
        <button
          ...
          className={cn(
            ...
            value === o ? 'bg-[#F0DFF6] text-[#5B2D8E]' : 'text-gray-500 hover:bg-gray-50',
          )}
        >
          {o}
          {counts?.[o] !== undefined && (
            <span className={cn('ml-1.5', value === o ? 'text-[#7E5896]' : 'text-gray-400')}>
              {counts[o]}
            </span>
          )}
        </button>
