...
import { AlertCircle, ArrowRight, CheckCircle2, CloudOff, Keyboard, Loader2, RefreshCw, Search, User, X } from 'lucide-react'
 ...
 * or unreadable.
 *
 * A sheet over the camera rather than a pushed route so the camera is never
 * torn down, and every admission here is flagged in the audit trail as
 * scan-less.
 *
 * Ported from apps/opus_pass_mobile/src/components/scanner/ManualCheckinSheet.tsx.
 * The native app's stacked-modal teardown sequencing (pendingResult /
 * onDismiss) is an iOS window-management concern; on the web these overlays
 * are plain conditional renders, so results hand up synchronously.
 */
export function ManualCheckinSheet({
  ...
  // Focus the right field when the sheet opens.
  useEffect(() => {
    if (visible) focusFor(initialMode)
  ...
  const submitCode = async (value: string) => {
    ...
    try {
      ...
      // success / duplicate are real answers about the guest: hand them to
      // the full-screen overlay, same as a scan.
      onClose()
      onAdmitted(result)
    } finally {
      setAdmitting(null)
  ...
  const admitGuest = async (guest: RosterEntry, arrived: number) => {
    ...
    try {
      ...
      // scan screen's full-screen overlay reports it exactly like a scan.
      setConfirming(null)
      onClose()
      onAdmitted(result)
    } finally {
  ...
  return (
    <Sheet open={visible} onClose={onClose} label={mode === 'code' ? 'Enter Pass ID or ticket code' : 'Find guest by name'}>
      <div className="flex items-center justify-between border-b border-black/8 px-5 py-4">
        <button type="button" onClick={onClose} className="text-sm text-[#1A1A1A]">
          Cancel
        </button>
        ...
        <button
          type="button"
          aria-label={mode === 'code' ? 'Search by name instead' : 'Enter Pass ID or ticket code instead'}
          onClick={() => {
            const next: Mode = mode === 'code' ? 'name' : 'code'
            setMode(next)
            focusFor(next)
          }}
          className="flex h-9 w-13 items-center justify-end"
        >
      ...
      {mode === 'code' ? (
        <div className="flex-1 overflow-y-auto px-5 pt-8">
          <p className="text-center text-sm leading-5 text-[#1A1A1A]/60">
            Type the Pass ID or ticket code printed under the QR on the guest&apos;s ticket. A Pass ID shows the guest
            for you to check before anyone is admitted.
          </p>
          ...
          <div className="relative mt-7 flex justify-center gap-2" onClick={() => codeInputRef.current?.focus()}>
            ...
            <input
              ...
              disabled={busyOnCode}
              enterKeyHint="go"
              aria-label="Pass ID or ticket code"
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-10 pt-4">
              {results.length === 0 ? (
                <div className="mt-14 flex flex-col items-center px-8">
                  ...
                  <p className="mt-3 text-center text-sm text-[#1A1A1A]/60">
                    {query ? `No guest matching “${query.trim()}”.` : 'No guests on this list yet.'}
                  </p>
                </div>
              ) : (
                results.map((item) => {
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="flex items-center gap-2">
                          ...
                          {item.isVip ? (
                            <span
                              className="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold uppercase text-[#1A1A1A]"
                        ...
                        {item.entryCode || arrived ? (
                          <span className="mt-1 flex items-center gap-2">
                            {item.entryCode ? (
                              <span className="text-[10px] font-bold tracking-[1px] text-[#1A1A1A]/60">{item.entryCode}</span>
                            ) : null}
                            {arrived ? (
                              <span className="truncate text-xs text-[#1A1A1A]/60">
                                Already in, {item.checkedInPartySize ?? item.partySize} of {item.partySize}
                              </span>
                            ) : null}
                          </span>
                        ) : null}
                      ...
                      {arrived ? <CheckCircle2 size={24} className="shrink-0 text-[#1B7F4C]" /> : <PartyBadge partySize={item.partySize} />}
                    </button>
                  )
                })
              )}
