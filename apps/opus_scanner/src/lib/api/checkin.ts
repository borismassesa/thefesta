...
import type { CheckinScanResult, ResolveCodeResult, ValidateSessionResult } from '@/types/checkin'
 ...
 *
 * Requests leave this origin via a Next.js rewrite proxy (see next.config.ts)
 * to the opus_pass deployment, so the browser only ever talks same-origin and
 * opus_pass needs no CORS surface. The proxy forwards the attendant's real IP
 * (x-forwarded-for), which the API's login rate limits key on.
 *
