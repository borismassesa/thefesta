 ...
 *
 * Ported from apps/opus_pass_mobile/src/lib/scannerRoster.ts.
 */
 ...
 */
const AVATAR_COLORS = ['#7E5896', '#2F7D74', '#9A5B7A', '#4A6FA5', '#8A6B1E', '#55703F'] as const
...
export function initialsOf(fullName: string): string {
  ...
  const last =
    usable.length > 1 ? initialCharOf(usable[usable.length - 1]) || (usable[usable.length - 1][0] ?? '') : ''
...
export function arrivedHeads(guests: RosterEntry[]): number {
  return guests.reduce((sum, g) => (g.checkedInAt ? sum + (g.checkedInPartySize ?? g.partySize) : sum), 0)
}
