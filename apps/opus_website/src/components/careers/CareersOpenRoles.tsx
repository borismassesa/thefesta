...
import { ArrowUpRight, Bookmark, MapPin, SlidersHorizontal } from 'lucide-react'
import {
  applyHref,
  formatCareerDate,
...
export default function CareersOpenRoles({ jobs }: { jobs: CareerJob[] }) {
  const [department, setDepartment] = useState(ALL)
  const [location, setLocation] = useState(ALL)
  const [employment, setEmployment] = useState(ALL)
  const [workplace, setWorkplace] = useState(ALL)
  const savedSnapshot = useSyncExternalStore(subscribeToSavedJobs, getSavedJobsSnapshot, () => '[]')
  const saved = useMemo(() => {
    try {
  ...
  const matches = grouped.reduce((total, [, departmentJobs]) => total + departmentJobs.length, 0)
  ...
  return (
    <section id="open-roles" className="scroll-mt-24 border-y border-black/10 bg-white">
      <div className="mx-auto max-w-[1240px] px-6 py-24 md:py-32">
        <div className="mb-14 grid gap-8 md:grid-cols-[1fr_auto] md:items-end">
          <div>
            ...
            <h2 className="max-w-2xl text-4xl font-medium leading-[1.05] tracking-[-0.04em] md:text-6xl">
              Find the work that feels like yours.
            </h2>
          </div>
          <p className="max-w-sm text-base leading-7 text-black/60 md:text-right">
            {jobs.length} {jobs.length === 1 ? 'role is' : 'roles are'} currently open across our teams.
          </p>
        </div>
        ...
        {jobs.length > 0 && (
          <div className="mb-14 rounded-[28px] border border-black/10 bg-[#F4F4F0] p-5 md:p-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button type="button" onClick={resetFilters} className="text-sm font-medium underline-offset-4 hover:underline">
                Reset
              </button>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <Filter label="Department" value={department} onChange={setDepartment} options={filters.departments} />
              <Filter label="Location" value={location} onChange={setLocation} options={filters.locations} />
              <Filter label="Employment" value={employment} onChange={setEmployment} options={filters.employment} />
              <Filter label="Workplace" value={workplace} onChange={setWorkplace} options={filters.workplace} />
            </div>
          </div>
        )}
        ...
        {matches === 0 ? (
          <div className="rounded-[36px] bg-[#191919] px-7 py-16 text-center text-white md:px-12">
            <p className="mb-3 text-2xl font-medium tracking-tight">
              {jobs.length === 0 ? 'No open roles right now.' : 'Nothing matches those filters.'}
            </p>
            ...
            <a href="#talent-community" className="inline-flex rounded-full bg-white px-6 py-3 text-sm font-semibold text-black">
            ...
            {grouped.map(([team, teamJobs]) => (
              <div key={team}>
                <div className="mb-5 flex items-end justify-between border-b border-black/15 pb-4">
                  <h3 className="text-2xl font-medium tracking-tight md:text-3xl">{team}</h3>
                  <span className="text-sm text-black/50">{teamJobs.length.toString().padStart(2, '0')}</span>
                </div>
                <div className="grid gap-3">
                  {teamJobs.map((job) => {
                    ...
                    return (
                      <article key={job.id} className="group rounded-[24px] border border-black/10 bg-[#F7F7F3] p-5 transition hover:border-black/25 hover:bg-[#F1F0EB] md:p-7">
                        <div className="flex items-start justify-between gap-5">
                          <Link href={applyHref(job)} className="min-w-0 flex-1 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4">
                            <div className="mb-3 flex flex-wrap items-center gap-2">
                              {urgency && (
                                <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.15em] ${urgency === 'closing-soon' ? 'bg-[#F8D5CF] text-[#7D2318]' : 'bg-[#DDEFCF] text-[#315C20]'}`}>
                                  {urgency === 'closing-soon' ? 'Closing soon' : 'New'}
                                </span>
                              )}
                              <span className="text-xs font-semibold uppercase tracking-[0.14em] text-black/45">{job.brand}</span>
                            </div>
                            <h4 className="text-xl font-medium tracking-tight md:text-2xl">{job.title}</h4>
                            <p className="mt-3 max-w-2xl text-sm leading-6 text-black/60">{job.summary}</p>
                            <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-sm text-black/55">
                              ...
                              {job.closingDate && <span>Closes {formatCareerDate(job.closingDate)}</span>}
                            </div>
                          </Link>
                          <button
                            type="button"
                        ...
                        <Link href={applyHref(job)} className="mt-6 inline-flex items-center gap-2 text-sm font-semibold">
                          View role <ArrowUpRight className="h-4 w-4 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
                        </Link>
                      </article>
                    )
                  })}
