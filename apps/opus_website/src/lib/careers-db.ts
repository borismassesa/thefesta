...
const EXTENDED_COLUMNS =
  'id, slug, title, department, location, employment_type, opened_at, posted_salary_min_tzs, posted_salary_max_tzs, description, brand, workplace_type, experience_level, closing_date, show_salary, summary, responsibilities, requirements, preferred_qualifications, working_conditions, recruitment_process';
...
function normalizeJob(row: JobRow): CareerJob {
  ...
  return {
    ...
    experienceLevel:
      ...
      (row.employment_type === 'Intern' ? 'Early career' : 'Professional'),
...
async function selectJobs(slug?: string): Promise<JobRow[]> {
  ...
  let postingQuery = supabase
    ...
    .eq('status', 'published');
  postingQuery = slug
    ? postingQuery.in('visibility', ['public', 'unlisted'])
    : postingQuery.eq('visibility', 'public');
  const { data: postings, error: postingError } = await postingQuery;
  if (postingError) throw postingError;
  const now = Date.now();
  const jobIds = (postings ?? [])
    .filter(
      (posting) =>
...
async function localizeJobs(
  ...
  if (!postingIds.length) return jobs;
  const { data: languages, error: languageError } = await supabase
    .from('recruitment_job_languages')
    .select(
      'posting_id, public_title, public_summary, public_description, responsibilities, requirements'
    )
    .in('posting_id', postingIds)
    .eq('language_code', locale)
    .in('status', ['approved', 'published']);
  if (languageError) throw languageError;
  const postingByJob = new Map(
    (postings ?? []).map((posting) => [posting.workforce_job_id, posting.id])
  );
  const languageMap = new Map(
    (languages ?? []).map((language) => [language.posting_id, language])
  );
  return jobs.map((job) => {
    const language = languageMap.get(postingByJob.get(job.id) ?? '');
    return language
      ? {
          ...job,
          title: language.public_title,
          summary: language.public_summary || job.summary,
          description: language.public_description || job.description,
          responsibilities: language.responsibilities?.length
            ? language.responsibilities
            : job.responsibilities,
          requirements: language.requirements?.length
...
export const loadOpenJobs = cache(
  async (locale = 'en'): Promise<CareerJob[]> => {
    try {
...
export const loadOpenJobBySlug = cache(
  async (slug: string, locale = 'en'): Promise<CareerJob | null> => {
...
export type CareerApplicationQuestion = {
  id: string;
  key: string;
  label: string;
...
export const loadApplicationQuestions = cache(
  async (jobId: string): Promise<CareerApplicationQuestion[]> => {
    const supabase = createSupabaseServerClient();
    ...
    if (postingError || !posting) return [];

    const { data, error } = await supabase
      .from('recruitment_application_questions')
      .select('id, key, label, help_text, question_type, options, is_required')
    ...
    return (data ?? []).map((question) => ({
      ...
      questionType:
        question.question_type as CareerApplicationQuestion['questionType'],
...
export const loadCareersCms = cache(
  async (locale = 'en'): Promise<CareersCmsContent | null> => {
    try {
      const supabase = createSupabaseServerClient();
      ...
      const [blocks, benefits, locations, faqs, stories] = await Promise.all([
        supabase
          ...
          .select('id, block_type, content, image_alt_text')
          .eq('page_id', page.id)
          .eq('is_visible', true)
          .order('sort_order'),
        supabase
          .from('careers_cms_benefits')
          .select('id, title, description, icon')
          .eq('status', 'published')
          .order('sort_order'),
        supabase
          .from('careers_cms_locations')
          .select('id, name, address, content')
          .eq('status', 'published')
          .order('sort_order'),
        supabase
          .from('careers_cms_faqs')
          .select('id, question, answer, category')
          .eq('status', 'published')
          .eq('locale', locale)
          .order('sort_order'),
        supabase
          .from('careers_cms_stories')
          .select('id, person_name, role_title, headline, body, image_url')
          .eq('status', 'published')
          .order('sort_order'),
      ]);
      for (const result of [blocks, benefits, locations, faqs, stories])
        if (result.error) throw result.error;
      return {
        page: {
          title: page.title,
        ...
        blocks: (blocks.data ?? []).map((block) => ({
          ...
          heading: String(block.content?.heading ?? ''),
          body: String(block.content?.body ?? ''),
          imageUrl: block.content?.image_url
            ? String(block.content.image_url)
            : null,
          imageAlt: block.image_alt_text,
          ctaLabel: block.content?.cta_label
            ? String(block.content.cta_label)
            : null,
          ctaHref: block.content?.cta_href
            ? String(block.content.cta_href)
            : null,
        })),
