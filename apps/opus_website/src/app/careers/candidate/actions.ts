...
async function validateDocument(
  file: File
): Promise<{ bytes: ArrayBuffer; extension: string; sha256: string }> {
  ...
  if (!valid)
    throw new Error('The file does not match its declared document type.');
  return {
    bytes,
    extension,
...
export async function updateCandidateContact(
  ...
  if (!candidate)
    throw new Error('No candidate profile is linked to this account.');
  ...
  const { error } = await supabase
    ...
    .eq('id', candidate.id)
    .eq('candidate_clerk_user_id', candidate.candidate_clerk_user_id);
...
export async function updateCandidatePreferences(
  ...
  const list = (name: string) =>
    value(formData, name)
      ...
      .filter(Boolean)
      .slice(0, 20);
  const min = value(formData, 'salary_min')
    ? Number(value(formData, 'salary_min'))
    : null;
  const max = value(formData, 'salary_max')
    ? Number(value(formData, 'salary_max'))
    : null;
  if (
    ...
    .upsert(
      {
        ...
        preferred_departments: list('departments'),
        preferred_locations: list('locations'),
        preferred_employment_types: list('employment_types'),
        salary_expectation_min: min,
        salary_expectation_max: max,
        remote_preference: value(formData, 'remote_preference') || null,
        earliest_start_date: value(formData, 'earliest_start_date') || null,
        willing_to_relocate: value(formData, 'willing_to_relocate') === 'on',
...
export async function updateCandidateConsent(
  consentType: 'talent_pool' | 'career_updates' | 'sms',
  intent: 'grant' | 'withdraw'
): Promise<void> {
  const candidate = await requirePortalCandidate();
  if (!candidate) throw new Error('Candidate profile not found.');
  ...
  if (intent === 'withdraw') {
    const { error } = await db
      ...
      .update({
        ...
        evidence: { candidate_portal: true, action: 'withdraw' },
      })
    ...
    const { data: existing } = await db
      ...
      .maybeSingle();
    if (!existing) {
      const { error } = await db.from('recruitment_candidate_consents').insert({
        candidate_id: candidate.id,
        consent_type: consentType,
        notice_version: 'candidate-portal-2026-08',
        granted_at: new Date().toISOString(),
        source: 'candidate_portal',
        evidence: { candidate_portal: true, action: 'grant' },
      });
      if (error) throw error;
    }
  }
  await db.from('recruitment_audit_events').insert({
    event_type: `candidate.consent_${intent}`,
...
export async function toggleCandidateSavedJob(
  jobId: string,
  intent: 'save' | 'remove'
): Promise<void> {
  const candidate = await requirePortalCandidate();
  if (!candidate)
    throw new Error(
      'Sign in with your verified candidate email to save roles.'
    );
  const db = createSupabaseServerClient();
  if (intent === 'save') {
    const { data: posting, error: lookupError } = await db
      .from('recruitment_job_postings')
      .select('id')
      .eq('workforce_job_id', uuid(jobId))
      .eq('status', 'published')
      .in('visibility', ['public', 'unlisted'])
      .maybeSingle();
    if (lookupError || !posting)
      throw new Error('This role is not available to save.');
    const { error } = await db
...
export async function withdrawCandidateApplication(
  ...
  const { error } = await supabase.rpc(
    'recruitment_candidate_withdraw_application',
    {
...
export async function submitCandidateAvailability(
  applicationId: string,
  formData: FormData
): Promise<void> {
  const candidate = await requirePortalCandidate();
...
export async function acknowledgeCandidateNotice(
  noticeId: string
): Promise<void> {
  const candidate = await requirePortalCandidate();
  if (!candidate) throw new Error('Candidate profile not found.');
...
export async function respondToBackgroundCheckConsent(
  taskId: string,
  consented: boolean
): Promise<void> {
  const candidate = await requirePortalCandidate();
  ...
  const { error } = await createSupabaseServerClient().rpc(
    'recruitment_candidate_respond_background_consent',
    {
      p_task_id: uuid(taskId),
      p_candidate_id: candidate.id,
...
export async function respondToCandidateOffer(
  ...
  const evidenceSecret =
    process.env.RECRUITMENT_EVIDENCE_SALT ||
    process.env.SUPABASE_SERVICE_ROLE_KEY;
  const ipHash =
    address && evidenceSecret
      ? createHmac('sha256', evidenceSecret).update(address).digest('hex')
      : null;
...
export async function requestCandidatePrivacyAction(
  formData: FormData
): Promise<void> {
  const candidate = await requirePortalCandidate();
  if (!candidate) throw new Error('Candidate profile not found.');
  ...
  if (
    !['access', 'correction', 'export', 'withdraw_consent', 'delete'].includes(
      requestType
    )
  ) {
...
export async function openCandidateOfferDocument(
  ...
  const { data: document, error } = await supabase
    ...
    .select(
      'id, storage_bucket, storage_path, offer_version_id, recruitment_offer_versions!inner(version)'
    )
  ...
  if (Number(version?.version) !== Number(offer.version))
    return { ok: false, error: 'That offer document is not available.' };
  ...
  await supabase.from('recruitment_audit_events').insert({
    event_type: 'offer.document_viewed',
    entity_type: 'offer',
    entity_id: offer.id,
    actor_type: 'candidate',
    metadata: { document_id: document.id, version: offer.version },
  });
  revalidatePath('/careers/candidate');
  return { ok: true, url: signed.signedUrl };
}

export async function submitCandidateAssessmentTask(
  taskId: string,
  _previous: PortalTaskState,
  formData: FormData
  ...
  try {
    const candidate = await requirePortalCandidate();
    if (!candidate)
      return { ok: false, message: 'Candidate profile not found.' };
    const answer = value(formData, 'answer');
    const attachment = formData.get('attachment');
    if (answer.length < 10)
      return {
        ok: false,
        message: 'Add a complete response before submitting.',
      };
    const supabase = createSupabaseServerClient();
    ...
    const { error } = await supabase.rpc(
      'recruitment_candidate_submit_assessment',
      {
        p_task_id: uuid(taskId),
        p_candidate_id: candidate.id,
        p_response: { answer },
        p_storage_paths: uploadedPath ? [uploadedPath] : [],
      }
    );
    if (error) throw error;
    revalidatePath('/careers/candidate');
    return {
      ok: true,
      message: 'Assessment submitted. The recruitment team will review it.',
    };
  } catch {
    if (uploadedPath)
    ...
    return {
      ok: false,
      message:
        'The assessment could not be submitted. Check the deadline and try again.',
    };
  }
}

export async function uploadCandidatePortalDocument(
  taskId: string | null,
  _previous: PortalTaskState,
  formData: FormData
): Promise<PortalTaskState> {
  ...
  try {
    ...
    if (uploadError) throw uploadError;
    const { error } = await supabase.rpc(
      'recruitment_candidate_register_document',
      {
        p_candidate_id: candidate.id,
        p_application_id: applicationId,
        p_task_id: taskId ? uuid(taskId) : null,
        p_document_type: documentType,
        p_storage_bucket: 'careers',
        p_storage_path: uploadedPath,
        p_original_filename: document.name.slice(0, 255),
        p_mime_type: document.type,
        p_byte_size: document.size,
    ...
    return {
      ok: true,
      message: 'Document uploaded securely. It will be scanned before review.',
    };
  } catch {
    if (uploadedPath)
      await createSupabaseServerClient()
...
export async function requestInterviewReschedule(
  ...
  if (reason.length < 5 || reason.length > 1000)
    throw new Error('Add a short rescheduling reason.');
  const supabase = createSupabaseServerClient();
  const { data: interview, error } = await supabase
    .from('recruitment_interviews')
    .select('id, application_id, recruitment_applications!inner(candidate_id)')
    .eq('id', uuid(interviewId))
  ...
  if (!interview || application?.candidate_id !== candidate.id)
  ...
  const { error: insertError } = await supabase
    ...
    .insert({
      ...
      requested_by_candidate_id: candidate.id,
      reason,
      proposed_slots: [],
    });
...
export async function requestInterviewAccommodation(
  interviewId: string,
  formData: FormData
): Promise<void> {
  const candidate = await requirePortalCandidate();
  ...
  if (details.length < 5 || details.length > 2000)
    throw new Error('Describe the accommodation requested.');
  const supabase = createSupabaseServerClient();
  const { data: interview, error } = await supabase
    .from('recruitment_interviews')
    .select('id, application_id, recruitment_applications!inner(candidate_id)')
  ...
  if (!interview || application?.candidate_id !== candidate.id)
    throw new Error('Interview not found.');
  const { error: insertError } = await supabase
    .from('recruitment_accommodation_requests')
    .insert({
      candidate_id: candidate.id,
