...
export async function generateMetadata({
  ...
  const job = await loadOpenJobBySlug(
    slug,
    (await searchParams).lang === 'sw' ? 'sw' : 'en'
  );
    ...
    <main>
      ...
      <header className="border-b border-black/10 bg-[#F4F4F0]">
        <div className="mx-auto max-w-[1200px] px-6 py-12 md:py-20">
          <Link
            href={`/careers?lang=${locale}#open-roles`}
          ...
          <div className="mt-12 grid gap-10 lg:grid-cols-[1fr_340px] lg:items-end">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-black/45">
                {job.department} · {job.brand}
              </p>
              <h1 className="mt-5 max-w-4xl text-5xl font-medium leading-[0.98] tracking-[-0.055em] md:text-7xl">
                {job.title}
              </h1>
            ...
            <Link
              href={`/careers/jobs/${job.slug}/apply?lang=${locale}`}
              className="inline-flex min-h-13 items-center justify-center gap-2 rounded-full bg-black px-7 text-sm font-semibold text-white"
            >
      ...
      <div className="mx-auto grid max-w-[1200px] gap-14 px-6 py-20 lg:grid-cols-[1fr_330px] lg:gap-20 lg:py-28">
        <div className="space-y-16">
          <ContentSection title="Why this role exists">
            <p className="text-lg leading-8 text-black/65">{job.description}</p>
          </ContentSection>
          <ContentSection title="What you will own">
            <BulletList items={job.responsibilities} />
          </ContentSection>
          <ContentSection title="What we are looking for">
            <BulletList items={job.requirements} />
          </ContentSection>
          {job.preferredQualifications.length > 0 && (
            <ContentSection title="Helpful, not essential">
              <BulletList items={job.preferredQualifications} />
            </ContentSection>
          )}
          {job.workingConditions.length > 0 && (
            <ContentSection title="Working conditions">
              <BulletList items={job.workingConditions} />
            </ContentSection>
          )}
          <ContentSection title="Your hiring journey">
            <ol className="border-t border-black/10">
              {job.recruitmentProcess.map((step, index) => (
                <li
                  key={step}
                  className="grid grid-cols-[42px_1fr] border-b border-black/10 py-4"
                >
          ...
          <section className="rounded-[var(--opus-radius-large)] bg-[#E3F0D6] p-7 md:p-9">
            ...
            <p className="mt-4 leading-7 text-black/65">
              We hire for skill, judgement and contribution. We do not
              discriminate on the basis of race, ethnicity, religion, gender,
              disability, age or any other protected characteristic. Tell us if
              you need an accommodation at any stage.
            </p>
            <p className="mt-4 text-sm font-semibold">
              OpusFesta never charges candidates recruitment fees.
            </p>
          </section>
        </div>
          ...
          <div className="rounded-[var(--opus-radius-large)] border border-black/10 bg-white p-6 shadow-[0_18px_50px_rgba(0,0,0,0.06)]">
            ...
            <dl className="mt-5 divide-y divide-black/10 text-sm">
              ...
              {job.closingDate && (
                <Fact
                  label="Apply by"
                  value={formatCareerDate(job.closingDate)}
                />
              )}
              {job.showSalary &&
                job.salaryMinTzs !== null &&
                job.salaryMaxTzs !== null && (
                  <Fact
                    label="Monthly range"
              ...
              <Fact
                label="Reference"
                value={`OF-${job.id.slice(0, 8).toUpperCase()}`}
              />
            </dl>
            <Link
              href={`/careers/jobs/${job.slug}/apply?lang=${locale}`}
              className="mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-black px-5 text-sm font-semibold text-white"
            >
...
function ContentSection({
  ...
  return (
    <section>
      <h2 className="mb-6 text-3xl font-medium tracking-[-0.03em] md:text-4xl">
        {title}
      </h2>
      {children}
    </section>
...
function BulletList({ items }: { items: string[] }) {
  return (
    <ul className="space-y-4">
      {items.map((item) => (
        <li key={item} className="flex gap-3 leading-7 text-black/65">
          <Check className="mt-1.5 h-4 w-4 shrink-0" />
          {item}
        </li>
      ))}
    </ul>
...
function Fact({
  ...
  return (
    <div className="flex gap-3 py-4">
      {Icon ? (
        <Icon className="mt-0.5 h-4 w-4 text-black/40" />
      ) : (
